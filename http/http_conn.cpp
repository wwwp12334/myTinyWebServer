...
bool HttpConn::selectName(MYSQL *mysql, char *name) {
    char sql[50] = {'\0'};
    strcat(sql, "select username from user where username = '");
    strcat(sql, name);
    strcat(sql, "'");
    //printf("sql:%s\n", sql);
...
bool HttpConn::selectAccount(MYSQL *mysql, char *name, char *passwd) {
    char sql[50] = {'\0'};
    strcat(sql, "select username, passwd from user where username = '");
    strcat(sql, name);
...
HttpConn::HTTP_CODE HttpConn::do_request() {
    ...
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3')) {
        ...
        if (*(p + 1) == '3') {
            char sql[200] = {'\0'};
            strcpy(sql, "INSERT INTO user(username, passwd) VALUES(");
            strcat(sql, "'");
            strcat(sql, name);
            strcat(sql, "', '");
            strcat(sql, passwd);
           ...
           // printf("bret:%d\n", bret);
            ...
            locker.lock();
            bool bres = selectAccount(mysql, name, passwd);
            locker.unlock();
...
bool HttpConn::write()
{
    ...
    if (bytes_to_send == 0)
    {
        modfd(m_epollfd, m_sockfd, EPOLLIN, m_TRIGMode);//writev按顺序发送内存块，如果没有一次性发送所有字节，会先返回发送的第一块内存的字节；
        init();
        return true;
    }
...
bool HttpConn::add_response(const char *format, ...) {
    ...
    int len = vsnprintf(m_write_buf + m_write_idx, WRITE_BUFFER_SIZE - 1 - m_write_idx, format, valst);
